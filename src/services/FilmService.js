...
export const getTypeFilm = async (type, data, page, limit) => {
  console.log(type, data, page, limit);

  let res;
  if (type === "the-loai") {
    res = await axios.get(`${process.env.REACT_APP_API_CATEGORY_FILM}${data}?page=${page}&limit=${limit}`);
  }
  if (type === "quoc-gia") {
    res = await axios.get(`${process.env.REACT_APP_API_COUNTRY_FILM}${data}?page=${page}&limit=${limit}`);
  }
  return res.data;
};

export const getAllTypeFilm = async () => {
  let res = await axios.get(`${process.env.REACT_APP_API_CATEGORY}`);
  return res.data;
};
export const getAllCountryFilm = async () => {
  let res = await axios.get(`${process.env.REACT_APP_API_COUNTRY}`);
  return res.data;
};
